...
Frequently Asked Questions about the I<nms> CGI programs ( $Date: 2001-11-25 16:01:11 $ )

=head1 DESCRIPTION

This document seeks to some of the questions that people might want to ask
about the I<nms> project and the programs therein.
...
CGI programs currently available on the web. Amongst the Perl community,
...
don't share our views and many people have learnt very bad coding habits
...
free to choose between the GNU Public License or the Artistic License.

=head2 But I have programs that do the same thing already, why change them ?

Because the programs you may already use aren't as good as these.
...
presentation inforamtion is stored in a stylesheet.
...
set ourselves the target that all of our progrma should exactly the same
...
C<use strict> (together with C<-w>) are a great way to keep a huge range
of bugs out of your scripts. Even though the I<nms> project team are all
very experienced Perl programmers, we all insist on using then in all of
our scripts.
...
C<cgi-lib.pl> was create for writing CGI programs back in the days of Perl 4,
...
It's certainly big! But becuase of the way it's written, only functions that
...
Yes, but it's also something that you should do everytime that you access
...
to the file simulataneously, then the file can become corrupted. If they
both try to flock (for "file lock") firest, then olny the first one will
be successful. The second will be forced to wait unti the first unlocks the
file. Thise leads to many less corrupt files and is therefore a good idea.
