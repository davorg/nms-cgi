...
Frequently Asked Questions about the I<nms> CGI programs ( $Date: 2002-03-14 21:33:59 $ )
...
set ourselves the target that all of our programs should work exactly the
same way as Matt's originals. If you set C<$emulate_matts_code> to 1, then
the I<nms> script will work exactly the the same way. If you set it to 0,
then our script will be free to do some things in a more sensible way, 
...
to say that is if one process has a file open so it can write to it and a
...
message.  This is alright for testing or development purpose where you are
unlikely to have more than person using the program at the same time, but
...
=head2 But I don't have C<sendmail> on my webserver, what can I do ?

Well, if your webserver has a Unix-like operating system it is likely that
...
If you are on a Windows webserver then you probably don't have a local MTA
at all, although this is not say that you will not be able to use C<FormMail>.
You should ask your System Administrator or web hosting provider is the
program C<blat> is available, C<blat> is command line program for Windows
that enables you to send e-mail in a similar way to C<sendmail>, if it is
not available you could get your own copy and upload it to your own website
at a push, however it does require some configuration so you should read the
documentation before you do that.  Microsoft did once make available a port
of C<sendmail> for Windows NT which works exactly the same as the Unix version
however it is extremely unlikey to be already installed on your webserver
- you should be able to find it on the microsoft FTP site but it is 
almost certainly going to be too difficult to install unless you are the
administrator of the server yourself.

Blat is available from  http://www.interlog.com/~tcharron/blat.html

Microsofts sendmail port is at 
...
A quick hack that we really don't think we would ever consider to put in 
