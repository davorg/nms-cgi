...
Frequently Asked Questions about the I<nms> CGI programs ( $Date: 2002-07-10 20:24:15 $ )
...
the queueing, MX lookup and retry capabilities of the local message transfer
...
As part of the I<nms> project a program called C<nms_sendmail> has been
released that provides the same function as sendmail.  This can be
downloaded from the same place that you downloaded FormMail and its
installation is described in the associated README file.
...
programs and more besides :)  On the other hand they every right to be
...
to use what is found there as program to be executed with the scriptname
