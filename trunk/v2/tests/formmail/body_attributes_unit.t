...
sub request
{
    ...
    {
        $self->{FormConfig} = { background => $t->[1] };
        my $ok = ($self->body_attributes =~ /background/ ? 1 : 0);
        if ($ok != $t->[0])
        {
            warn "body background [$t->[1]] ",
                 ($ok ? 'accepted' : 'rejected'),
                 " in error (" . $self->body_attributes . ")\n";
        }
    }
    ...
    foreach my $t (@color_tests)
    {
        ...
        {
            $self->{FormConfig} = { $attrib->[0],  $t->[1] };
            my $ok = ($self->body_attributes =~ /$attrib->[1]/ ? 1 : 0);
            if ($ok != $t->[0])
            {
