#!/usr/bin/perl -wT
#
#   $Id: guestbook-admin.pl,v 1.1 2004-05-07 09:09:03 gellyfish Exp $
#
...
sub is_old_cookie
{
    ...
    return 1 if (stat $session_dir . $filename)[10] < ($now - (6 * 60 * 60));
}
...
sub add_admin_html
{
    print qq%<a href="$myURL?action=delete&entry=$comment_num&cookie=$inputs{cookie}">delete comment</a>\n%;
    print qq%<a href="$myURL?action=hide&entry=$comment_num&cookie=$inputs{cookie}">hide/unhide comment (currently %;
...
sub view_guestbook_as_admin
{
    ...
    while (<$in>){
        ...
        if (/<\/body>/){
            print qq%<a href="$myURL?action=logout&cookie=$inputs{cookie}">Logout</a>\n%;
        }
        print $_ . "\n" unless /^-->/;
        if (/^<!-- comment end -->$/){
            &add_admin_html(); 
...
sub process_login
{
    ...
    opendir SESSION_DIR, $session_dir or 
        die "could not open session directory";
    ...
    if ($password and $password eq $inputs{password}){
        ...
        my $cookiename = unpack 'H*', join '', map {chr int rand 256} (1 .. 20);

        # create cookie file on disk, avoiding exist/create race
        sysopen(SAVED, $session_dir . $cookiename,
            O_CREAT | O_WRONLY | O_EXCL) or 
            die $session_dir . $cookiename .  ": $!";

        print SAVED "1";
        close SAVED;

        # send cookie to user
        print CGI::header(-type=>"text/html",status=>"302 Moved",
...
sub do_toggle_hidden
{
    ...
    my $lock = IO::File->new(">>$guestbookreal.lck") or die 
        "open $guestbookreal.lck: $!";
    flock $lock, LOCK_EX or die "flock $guestbookreal: $!@";

    my $temp = IO::File->new(">$guestbookreal.tmp") or die
        "open >$guestbookreal.tmp: $!";
    ...
    rename "$guestbookreal.tmp", $guestbookreal or die
...
# first, make sure we send content out as soon as it's printed. no buffering
# stuff we don't want buffered!
$|=1;
